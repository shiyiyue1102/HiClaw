...
| 5 | Go Controller + kine 引擎（项目骨架 + CRD + Reconciler） | ✅ 骨架 |
| 6 | 容器外入口脚本（hiclaw-apply.sh, hiclaw-import.sh 改造） | ✅ |
| 7 | 文档与协议更新 | ✅ |

### 后续规划

- hiclaw-controller 完整 reconcile 闭环（补全嵌入式 API Server 集成）
- `hiclaw get` / `hiclaw delete` 完整实现
- `--watch` 模式（fsnotify 文件监听）
